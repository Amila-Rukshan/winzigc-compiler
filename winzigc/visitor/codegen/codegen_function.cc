...
void CodeGenVisitor::visit(const Frontend::AST::Function& function) {
...
llvm::FunctionType* CodeGenVisitor::codegen_func_dcln(const Frontend::AST::Function& function) {
...
void CodeGenVisitor::codegen_func_def(const Frontend::AST::Function& function) {
  ...
  for (auto& param : llvm_function->args()) {
    ...
    if (debug) {
      llvm::DILocalVariable* debug_param = debug_builder->createParameterVariable(
          sub_program, function.get_parameters().at(param_index)->get_name(), param_index + 1, unit,
          line_number, debug_get_type(function.get_parameters().at(param_index)->get_type()), true);
...
CodeGenVisitor::debug_create_function_type(const Frontend::AST::Function& function) {
